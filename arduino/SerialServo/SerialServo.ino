    ...
    if (mode == 1)
    {
      if (!digitalRead(KEY2))
      {
        ...
        if (!digitalRead(KEY2))
        {
          ...
          Serial.print("pos=");Serial.println(pos1[step-1]);
          if (step == 4)
            step = 0;
          delay(500);
        }
...
void receiveData(int byteCount){
  ...
  anglePos = number;
  Serial.print("\n\n");
  LobotSerialServoMove(Serial, ID1, anglePos, 500);
}
  
  // callback for sending data
void sendData(){
  readAngle = LobotSerialServoReadPosition(Serial, ID1);
  Serial.print("sending data: ");
  Serial.println(readAngle);
  Wire.write((char*) &readAngle,2);
}
